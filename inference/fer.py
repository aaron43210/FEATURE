...
try:
    ...
    print(f"Error importing {e.name}. Install GDAL/Shapely.")
...
def latlon2utmxy(lat_rad, lon_rad, zone):
    """Simple pyproj wrapper for lat/lon to UTM."""
    utm_crs = (
        pyproj.CRS.from_epsg(32600 + zone)
        if lat_rad >= 0
        else pyproj.CRS.from_epsg(32700 + zone)
    )
    transformer = pyproj.Transformer.from_crs("epsg:4326", utm_crs, always_xy=True)
    x, y = transformer.transform(rad2deg(lon_rad), rad2deg(lat_rad))
    return x, y


def utmxy2latlon(x, y, zone, southhemi):
    """Simple pyproj wrapper from UTM back to lat/lon."""
    utm_crs = pyproj.CRS.from_epsg(32700 + zone if southhemi else 32600 + zone)
...
def regularize_polygon_shapely(
    poly: Polygon, min_length: float = 2.0, min_area: float = 10.0
) -> Polygon:
    """
    Wrapper for regularize that takes a Shapely Polygon and returns a Shapely Polygon,
    doing the OGR conversion in memory without hitting the disk.
    """
    ...
    # 1. Convert Shapely -> OGR
    wkt = poly.wkt
    igeom = ogr.CreateGeometryFromWkt(wkt)
    if igeom is None:
        return poly
    ...
    # 2. Re-implement the core regularize loop on the single geometry
    for j in range(iringCount):
        iring = igeom.GetGeometryRef(j)
        delepolygon.AddGeometry(iring)
        iringpts = Ring2Pts(iring)
        ...
        ideleteIds = []
        ideleteIds = compress(iringpts[0], iringpts[-1], iringpts, ideleteIds)

        dringpts = [k for k in iringpts if k.index not in ideleteIds]
        ...
        averageLength = (
            dpolygon.Boundary().Length() / dpolygon.GetGeometryRef(0).GetPointCount()
        ) * 1.5
        area = dpolygon.Area()
        ...
        while m <= min_length:
            ...
            svList = LocalResc(svList, vx, 20, m, averageLength, area)
            oring = vList2Ring2(svList)
            oring, _ = SelfIntersection(oring)
    ...
    # 3. Convert OGR -> Shapely
    if oupolygon.IsEmpty():
        return poly

    try:
        from shapely import wkt as shapely_wkt

        result_poly = shapely_wkt.loads(oupolygon.ExportToWkt())
        if result_poly.is_valid and not result_poly.is_empty:
            return result_poly
    except Exception:
        pass
