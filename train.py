...
def parse_args():
    ...
    p.add_argument(
        "--batch_size", type=int, default=16, help="Batch size for segmentation"
    )
...
def main():
    ...
    yolo_prep_cmd = (
        ...
        + args.train_dirs
        + ["--output", "yolo_dataset", "--tile_size", "1024"]
    )
    ...
    yolo_train_cmd = [
        ...
        "--data",
        "yolo_dataset/svamitva_points.yaml",
        "--epochs",
        str(args.epochs),
        "--batch",
        str(args.yolo_batch),
        "--project",
        "checkpoints/yolo_runs",
    ]
    ...
    yolo_best = (
        project_root / "checkpoints" / "yolo_runs" / "train" / "weights" / "best.pt"
    )
    if yolo_best.exists():
        import shutil
        ...
        logger.info(
            f"📍 Unified YOLO weights copied to {check_dir / 'yolo_svamitva_best.pt'}"
        )
